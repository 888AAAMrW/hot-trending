...
以下是所有热搜标题列表。请找出与搜索词语义相关的结果（不只是字面匹配，要理解含义）。例如搜索"手机"应匹配"iPhone""华为""折叠屏"等；搜索"战争"应匹配"冲突""军事""导弹"等。

返回格式：每行一个编号，按相关度从高到低排列，最多 20 条。只输出编号，不要解释。
        ...
        messages: [
          {
            role: "system",
            content:
              "你是一个语义搜索引擎。根据用户搜索意图，从标题列表中找出语义相关的结果。只输出编号，每行一个，按相关度排序。最多 20 条。不要输出任何解释。",
          },
